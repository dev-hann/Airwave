...
- `app/api/routes.py` is a 45-line aggregator mounting **19 domain sub-routers** under `/api`. Routers import the repository facade only (store internals forbidden — enforced by `tests/test_architecture.py`).
