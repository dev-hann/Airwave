...
| `playlists` | `playlists` (Playlist[]), duplicate-modal state | CRUD, `importPlaylistUrl`, `startSpotifyImportFromUrl`, `importPlaylistIntoPlaylist`, `addUrlToPlaylist`, `addEntriesToPlaylist`, `addLocal*ToPlaylist`, reorders | All duplicate-check flows go through **one** helper: `withDuplicateCheck(url, body, toasts, errorTitle, onComplete?)` — check → modal → `add_all`/`skip_duplicates`. Never copy the pattern again. |
